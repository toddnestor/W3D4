...
def eighties_b_movies
  ...
  Movie.where("yr BETWEEN 1980 AND 1989 AND score BETWEEN 3 AND 5")
...
def starring(whazzername)
  ...
  whazzername = whazzername.downcase.gsub(' ', '')
  search_string = "%#{whazzername.chars.join('%')}%"

  Movie.joins(:actors)
       .where("LOWER(actors.name) LIKE '#{search_string}'")
       .load
end

def bad_years
  # List the years in which a movie with a rating above 8 was not released.
  good_years = Movie.where("score > 8").select(:yr).uniq
  good_years.to_a.map! { |year| year.yr }

  Movie.where("yr NOT IN (#{good_years.join(",")})").select(:yr).uniq
  ## use .group and .having (& .pluck)
end
